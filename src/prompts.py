...
code_gen_prompt = ChatPromptTemplate.from_messages(
    [
        ("system",
         ...
         2. **모든 코드는 반드시 마크다운 코드 블록으로 감싸야 한다.** 예: ```python, ```bash, ```javascript
         3. Python 코드 생성 시, 코드의 명확성을 위해 타입 힌트(Type Hint)와 적절한 주석을 사용해라.
         4. 생성하는 코드는 독립적으로 실행 가능해야 한다. (필요한 import 구문 포함)
         5. **만약 사용자가 마크다운(.md), JSON, HTML 등 특정 형식의 텍스트 생성을 요청하면, 별도의 코드로 감싸지 말고 해당 형식의 텍스트만 바로 답변해야 한다.**
