...
- Requests is [installed](https://requests.readthedocs.io/en/latest/user/install/#install)
- Requests is [up-to-date](https://requests.readthedocs.io/en/latest/community/updates/#updates)
...
Now, we have a [**`Response`**](https://requests.readthedocs.io/en/latest/api/#requests.Response) object called `r`. We can get all the information we need from this object.
...
`payload_tuples = **[(**'key1'**,** 'value1'**),** **(**'key1'**,** 'value2'**)]**r1 = requests.post**(**'https://httpbin.org/post'**,** data=payload_tuples**)**payload_dict = **{**'key1'**:** **[**'value1'**,** 'value2'**]}**r2 = requests.post**(**'https://httpbin.org/post'**,** data=payload_dict**)**print**(**r1.text**)**{
  ...
  "form": {
    "key1": [
      "value1",
...
files = **{**'file'**:** open**(**'report.xls'**,** 'rb'**)}**r = requests.post**(**url**,** files=files**)**r.text
{
  ...
  "files": {
...
`url = 'https://httpbin.org/post'
files = **{**'file'**:** **(**'report.xls'**,** open**(**'report.xls'**,** 'rb'**),** 'application/vnd.ms-excel'**,** **{**'Expires'**:** '0'**})}**r = requests.post**(**url**,** files=files**)**r.text
{
  ...
  "files": {
    "file": "<censored...binary...data>"
...
For sending multiple files in one request refer to the [advanced](https://requests.readthedocs.io/en/latest/user/advanced/#advanced) section.
...
`r = requests.get**(**'https://httpbin.org/get'**)**r.status_code
200`
...
`bad_r = requests.get**(**'https://httpbin.org/status/404'**)**bad_r.status_code
404

bad_r.raise_for_status**()Traceback (most recent call last):**
...
`jar = requests.cookies.RequestsCookieJar**()**jar.set**(**'tasty_cookie'**,** 'yum'**,** domain='httpbin.org'**,** path='/cookies'**)**jar.set**(**'gross_cookie'**,** 'blech'**,** domain='httpbin.org'**,** path='/elsewhere'**)**url = 'https://httpbin.org/cookies'
...
The [**`Response.history`**](https://requests.readthedocs.io/en/latest/api/#requests.Response.history) list contains the [**`Response`**](https://requests.readthedocs.io/en/latest/api/#requests.Response) objects that were created in order to complete the request. The list is sorted from the oldest to the most recent response.
...
Ready for more? Check out the [advanced](https://requests.readthedocs.io/en/latest/user/advanced/#advanced) section.